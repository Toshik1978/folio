<template>
  <div>
    <h1 class="mb-4 text-[22px] font-bold">Authors</h1>
    <AlphabetSelector
      :active-letter="activeLetter"
      :available-letters="availableLetters"
      @select="selectLetter"
    />
    <AlphabetList :items="items" filter-key="author" />
...
const { items, availableLetters, activeLetter, loading, selectLetter } = useAlphabetBrowse<Author>(
  scrollTrigger,
  fetchAuthorLetters,
  fetchAuthors,
);
